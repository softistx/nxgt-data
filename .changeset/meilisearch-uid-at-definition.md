...
A literal `'*'`, `''`, or one holding a space, a dot or a slash no longer compiles. A uid typed `string`, a generic uid, a union of literals, or one holding anything else, still compiles and is checked at run time only.
