...
| A primary key in an update's patch | refused before anything is sent: an `ArgumentError`, and the addressed key is out of `UpdatePatch`/`ManyPatch`; every other key column is refused at run time only | `_id` refused before anything is sent (since 0.18.0): a bare `TypeError`, and `_id` is out of every patch type, operators included | the same rule; the class is each package's own, and there `_id` is the only key and always known to the types |
