...
- **`@nxgt/redis-guard`'s specs run against a real Redis** too, from the
  ...
  Idempotency has five, in `src/idempotency/`: `define-idempotency` (the
  ...
  calling `work` exactly once, and a **real** 30 ms lease lapsing — the
  late run gets `LEASE_LOST`, cannot store over the run that took the key
  since, and cannot release it — and `stored-state`, a hash per way a record
  can be one `run` did not write, each `INVALID` with the record kept and
  `work` not called, plus `WRONGTYPE`. Nothing there reads a clock, so no
  ARGV variant was needed: the lapse specs wait, bounded, for the key to go
