...
- **Specs are split by subject, not one per source file.** `@nxgt/drizzle`'s
  ...
  three the parity with `@nxgt/mongo` added, with their 30 tests, took the
  suite from **9.1-10.1 s** to **12.2-12.9 s** over three runs each; `@nxgt/s3` was left whole because the same measurement
...
`bun run test` is **1224 pass, 0 fail**: drizzle 143, meilisearch 81,
