...
- **`@nxgt/redis`'s specs run against a real Redis**, one per spec file,
  ...
  use instead. CI caches `.cache/redis`, keyed on **all four** copies of
  the server — `@nxgt/redis`'s, `@nxgt/redis-kit`'s and
  `@nxgt/redis-guard`'s `test/server.ts`, and `examples/hono-api/test/redis.ts`
  — and the script. The example starts one too, and its `test` script runs
  `scripts/redis.ts` first as the packages' do. `@nxgt/redis`'s `test/fixtures.ts` calls `closeRedis()` before stopping the server,
...
| `test/server.ts` of `@nxgt/redis`, copied into `@nxgt/redis-kit` and `@nxgt/redis-guard`, and once more as `examples/hono-api/test/redis.ts` | the same rule: a package reaches no sibling's tests, a kit over a sibling is a package like any other, and an example reaches no package's. Keep `REDIS_VERSION` equal in all four files — `redis-memory-server` **compiles** the source, so two versions is two builds and two caches, and CI keys the redis cache on the hash of the four files and `scripts/redis.ts`. No copy adds anything to the server itself; what differs is each `test/fixtures.ts` — `@nxgt/redis-kit`'s also closes the kits a spec opened, and `@nxgt/redis-guard`'s calls no `closeRedis()`, since it never connects through `@nxgt/redis`. The example's is the shortest: it has no `redisBinary` (the build is `scripts/redis.ts`'s, which imports `@nxgt/redis`'s), no `reset` on the server, and carries its own `useRedis` fixture beside `startRedis`, emptying the server with `FLUSHDB` before each test, the way `test/kit.ts` carries `useKit` beside its mongod |
...
`bun run test` is **1419 pass, 0 fail**: drizzle 152, meilisearch 117,
...
hono-api-example 41, scripts 21. hono-api-example's 41 was measured on its
own (three runs), as redis-guard's 124 was; the total is computed from
develop's 1409 with the example's 31 replaced, not measured by a full run. It runs one process
